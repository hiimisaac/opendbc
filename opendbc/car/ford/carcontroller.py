...
from opendbc.car.ford.coherent_path import (
  FORD_COHERENT_ANGLE_ERROR_DEADZONE_DEG,
...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP, CP_SP):
    ...
    self.path_controller = CoherentPathController()
    self.model = None
    self.model_frame = 0
    self.sm = None
  ...
  def update(self, CC, CC_SP, CS, now_nanos):
    ...
    if (self.frame % CarControllerParams.STEER_STEP) == 0:
      ...
      if self.CP.flags & FordFlags.CANFD:
        ...
        angle_error_deg = math.copysign(max(abs(angle_error_deg) - FORD_COHERENT_ANGLE_ERROR_DEADZONE_DEG, 0.0),
                                        angle_error_deg)
        angle_error_curvature = -self.VM.calc_curvature(math.radians(angle_error_deg), CS.out.vEgoRaw, 0.0)
        measured_curvature = wheel_curvature if driver_override else current_curvature
        cmd = self.path_controller.update(model, desired_curvature, measured_curvature,
                                          CS.out.vEgoRaw, CC.latActive, driver_override,
                                          angle_error_curvature)
        cooperative_control = cmd.cooperative_control
