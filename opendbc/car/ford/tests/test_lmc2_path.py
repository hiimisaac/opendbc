...
def test_small_steady_curvature_stays_on_c2_only():
  assert not should_use_path_fallback(0.002, 0.0, 0.0, True)
...
def test_large_curvature_uses_path_with_c2_zero():
  ...
  assert should_use_path_fallback(0.0041, 0.0, 0.0, True)
...
def test_rapid_curvature_change_uses_path_with_c2_zero():
  assert should_use_path_fallback(0.001, 0.021, 0.0, True)
  assert math.isclose(c2_memory_decay_step(0.002, True).command, 0.0)


def test_stale_c2_memory_keeps_path_fallback_while_unwinding():
  assert should_use_path_fallback(0.001, 0.0, 0.003, True)
  assert should_use_path_fallback(-0.002, 0.0, 0.002, True)


def test_path_fallback_resets_inactive():
  assert not should_use_path_fallback(0.01, 1.0, 0.01, False)
