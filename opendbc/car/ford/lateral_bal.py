...
def lightweight_path_from_model(model, desired_curvature: float, current_curvature: float, v_ego: float,
                                path_angle_last: float, lat_active: bool) -> tuple[float, float]:
  """Return Ford c0/c1 from the model path plus bounded curvature correction.
  ...
  Desired curvature carries openpilot's centering/correction terms, so fold the
  model-vs-desired curvature residual back into the path polynomial instead of
  using Ford's sticky c2 curvature slot.
  """
  ...
  model_curvature = path_angle / max(d_look, 1.0)
